...
> 🔒 **No backend. No telemetry. No "log in with your soul."** Your data lives on your machine in plain old IndexedDB, and the only thing Yesterlog ever phones is the Jira site *you* point it at.
...
- **Local AI is on-device only.** When you enable Day Reconstruction's optional model, Yesterlog talks to [Ollama](https://ollama.com) on `localhost` (default `:11434`) through the Electron main process — your commits, diffs, and ticket text are summarised locally. No cloud, no API key, no telemetry.
