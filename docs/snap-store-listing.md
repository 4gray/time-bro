...
Yesterlog is a private, local-first desktop companion for weekly Jira time
tracking.
...
core works offline. If you enable the optional AI layer, Yesterlog talks only to
your own Ollama server on `localhost`; no cloud model or Yesterlog backend is
involved.
