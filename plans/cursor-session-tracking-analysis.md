...
**Origin:** Follow-up to the shipped "Open in Cursor" deeplink feature ([ticket-details-dialog.md], `shared/cursorDeeplink.ts`). Question: since Yesterlog is a tracking app, can we track a Cursor coding session, bind it to a Jira ticket afterward, and capture time (and maybe tokens)?
...
- **Time-on-ticket is Yesterlog's actual job.** Auto-capturing "you spent 47 min in a Cursor session on branch `feat/FTDM-404`" → one-click worklog directly attacks the core pain (forgetting to log). Yesterlog is uniquely positioned: it's the only tool sitting on **both** sides (Jira + the editor session). The branch name (or the ticket stamped at deeplink-send time) is the join key — no native Cursor "ticket" concept needed.
...
1. **Primary — Hooks → passive time → suggested worklog.** ~30-line hook writes `{conversation_id, branch, start, end, duration_ms}` JSONL on `sessionStart`/`sessionEnd`; Yesterlog tails it, maps branch→ticket, surfaces *"Log 47m to FTDM-404?"*. Official, passive, on-mission. **Build this first.**
2. **Secondary — MCP server for active logging.** Tiny Yesterlog MCP (`start_timer(ticket)`, `log_work(ticket, minutes)`, `fetch_ticket(key)`) to log from inside Cursor by intent. Cheap, official, cross-IDE complement. (A "skill"/rule is just packaging on top of this, not a data source.)
...
1. Ship a `.cursor/hooks.json` + capture script template (Yesterlog can write it for the user, or document it).
2. Hook on `sessionStart`/`sessionEnd`: stamp wall-clock, read git branch, append a JSONL line keyed by `conversation_id`.
3. Yesterlog ingestion: tail the JSONL, map branch → Jira key (reuse existing ticket resolution), aggregate session durations per ticket per day.
