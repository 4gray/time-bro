...
Rename the local Jira worklog tracker to TimeBro and refresh the app icon in the TimeBro clock/watch style.

## Decisions

- Use `TimeBro` for visible app and package branding.
...
- Expose the new preload bridge as `window.timeBro`, while keeping `window.jiraWeekTracker` as a compatibility alias.
- Move theme persistence to `timebro-theme`, with a fallback read from the legacy `sprintf-theme` key.
...
- `agent-browser` packaged app check against `release/mac-arm64/TimeBro.app` with CDP on port `9333`
