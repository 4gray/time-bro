...
- Treat Yesterlog's configured daily target as the allocation capacity; account for ordinary Jira worklogs already present on each day before filling bulk time.
...
- Yesterlog continues to create one Jira worklog. Durations longer than a day are allowed and receive an exact deterministic local projection; creating multiple Jira worklogs remains out of scope until explicitly approved.
...
- Scope Yesterlog's explicit allocation preferences by normalized Jira site plus account as well as worklog ID.
...
- Retry a Yesterlog allocation preference after the first successful sync supplies current site/account context.
...
- Treat imported worklogs as bulk only from two daily targets upward; shorter overtime stays on Jira's original date unless Yesterlog captured an explicit distribution choice.
...
- Merge legacy cached raw worklogs into the site-scoped ledger view so older weeks outside the first new scan window remain visible after context activation.
...
- Added explicit end-date/start-date direction when Yesterlog creates a bulk worklog; the preference stays local and Jira receives one worklog.
...
- Status: PR #17 is ready for review; nineteen automated review threads are addressed locally, current `origin/main` is integrated, and local verification is green. Final commit, CI/re-review, plus merge remain.
