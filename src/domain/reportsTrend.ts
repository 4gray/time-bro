...
export const buildReportsHistory = (
  ...
  const composition: CompositionWeek[] = weekStates.map((week) => {
    // Notes tagged "meeting" (ad-hoc syncs, 1:1s) count toward meetings rather
    // than firefighting — same split the day rings use, kept consistent here.
    const meetingNoteHours =
      week.days.reduce(
        (sum, day) =>
          sum +
          day.personalNotes.reduce(
            (noteSum, note) => noteSum + (note.category === "meeting" ? note.timeSpentSeconds : 0),
            0
          ),
        0
      ) / 3600;
    const ticketHours = week.jiraTrackedWeekHours;
    const meetingHours = week.recurringTrackedHours + meetingNoteHours;
    const fireHours = Math.max(0, week.personalNoteHours - meetingNoteHours);
    return {
      weekKey: week.weekKey,
      label: weekLabel(week.weekKey),
      ticketHours,
