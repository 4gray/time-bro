...
describe("redactForCloud", () => {
  ...
  it("maps ticket keys to stable placeholders and restores them in a response", () => {
    const { text, restore } = redactForCloud("Reviewed YLOG-395 and YLOG-395, then OPS-77.");
    expect(text).not.toContain("YLOG-395");
    ...
    expect(restore("Worked on TICKET-1 and TICKET-2.")).toBe("Worked on YLOG-395 and OPS-77.");
  });

  it("restores placeholders case-insensitively (models sometimes lower-case them)", () => {
    const { restore } = redactForCloud("YLOG-42");
    expect(restore("finished ticket-1 today")).toBe("finished YLOG-42 today");
  });
  ...
  it("replaces reversible tokens (signal ids) with ID-n and restores them", () => {
    ...
    const prompt = `[{"id":"${ids[0]}","key":"YLOG-1"},{"id":"${ids[1]}","key":"OPS-2"}]`;
    ...
    expect(restore('{"id":"ID-1","key":"TICKET-1"}')).toBe(`{"id":"${ids[0]}","key":"YLOG-1"}`);
  });
  ...
  it("reports how much was redacted and is a no-op on clean text", () => {
    ...
    const dirty = redactForCloud("YLOG-1 https://x.y jane@z.io");
