...
vi.mock("../components/TicketsView", () => ({
  TicketsView: (props: Record<string, unknown>) => {
    ...
    return (
      ...
      >
        <button type="button" onClick={() => (props.onToggleFavorite as (key: string) => void)("YLOG-101")}>
          favorite
        </button>
...
const closedTicket: JiraTicket = {
  ...
  url: "https://example.atlassian.net/browse/YLOG-102"
};
...
const baseProps = (): AppTicketsRouteProps => ({
  ...
  favoriteKeys: ["YLOG-101"],
  hoursByKey: { "YLOG-101": 2 },
  weekHoursLogged: 12,
  isConfigured: true,
...
describe("AppTicketsRoute", () => {
  it("maps loaded ticket state to TicketsView props", () => {
    ...
    expect(ticketsViewProps[0]?.favoriteKeys).toEqual(["YLOG-101"]);
    expect(ticketsViewProps[0]?.hoursByKey).toEqual({ "YLOG-101": 2 });
  ...
  it("passes TicketsView actions through unchanged", () => {
    ...
    expect(toggleFavorite).toHaveBeenCalledWith("YLOG-101");
