...
const ghostSignal: ReconstructSignal = {
  ...
  confidence: "med",
  startHour: 14,
  naiveDescription: "Worked on YLOG-500"
};
...
const touchedTicket: JiraTicket = {
  ...
  url: "https://elevait.atlassian.net/browse/YLOG-401"
};
...
describe("TodayView calendar", () => {
  ...
  it("keeps the rail's touched-today tickets with their Jira links", () => {
    ...
    expect(markup).toContain("Open YLOG-401 in Jira");
    expect(markup).toContain("https://elevait.atlassian.net/browse/YLOG-401");
  });
  ...
  it("renders the active-work dock with Today-specific logging guidance", () => {
    ...
    expect(markup).toContain("Log time for YLOG-401 today");
  });
  ...
  it("renders detected-but-unlogged activity as a ghost block", () => {
    ...
    expect(markup).toContain("YLOG-500");
    expect(markup).toContain("Detected coding session");
  });

  it("does not ghost activity for a ticket already logged today", () => {
    // The worklog fixture logs YLOG-397, so a signal for it must be suppressed.
    const loggedSignal: ReconstructSignal = { ...ghostSignal, id: "sig-2", key: "YLOG-397" };
